...
export class FitbitController extends BasePlatformController {
    ...
    public getStepTimeSeries(searchDate): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            ...
            console.log('########### getStepTimeSeries:', params);
            ...
            this.getResource('steps', params).then(data => {
                ...
                if (data.code === ResponseCode.OK) {
                    ...
                    const infos = data.data['activities-steps'];
                    ...
                    if (infos[0].value){
                        const value = infos[0].value;
                        const model = new StepModel();
                        if (value) {
                            model.step = Number(value);
                            ...
                            HeaderModel.unit = 'step';
                            HeaderModel.lifelogTypeGrpCd = 'LIFELOG_GRP_ACTIVE';
                            HeaderModel.lifelogTypeCd = 'LIFELOG_TYPE_STP';
                            HeaderModel.measureDt = DateUtil.convertFormat(new Date()); // 현재시간
...
/*
                    ...
                    if (data.data['activities-steps-intraday']){
                        const dataset = data.data['activities-steps-intraday'].dataset;
                        dataset.forEach(item => {
                            const model = new StepModel();
                            if (item.value !== 0) {
                                model.step = Number(item.value);
